...
(function (root, factory) {
    ...
    function analyze(input) {
        ...
        if (foods.length) {
            if (knownFiber.length===items.length && fiber<25) add('fiber',50,'Клітковина',`У внесених продуктах ${round(fiber)} г клітковини. Загальний орієнтир для дорослих — від 25 г на день.`, 'Якщо це весь раціон дня, додай бобові, цільнозернові продукти або овочі відповідно до переносимості. Збільшуй клітковину поступово.','tab-food',WHO);
            ...
            if (number(targets.p)>0 && protein<targets.p*.9) add('protein',55,'Білок у записах',`Внесено ${round(protein)} г із заданої в профілі цілі ${round(targets.p)} г.`, 'Якщо плануєш ще прийом їжі, включи джерело білка: яйця, рибу, кисломолочний продукт або бобові. Неповний журнал не доводить нестачу білка.');
        ...
        const limitations=[`${dateLabel}: оцінено лише внесені записи, повнота дня не підтверджена.`];
